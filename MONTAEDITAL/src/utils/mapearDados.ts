...
export const mapearDadosWizard = async (dados: any) => {
  ...
  return {
    dadosMapeados: {
        ...
        "{{DATA AUT}}": dados.dataAutorizacao || "",
        "{{SEC}}": dados.secretaria || "",
        "{{DATA_TR}}": dados.dataTr || "",
        "{{SERVIDOR}}": dados.servidorProcedimento || "",
        "{{DATA.MODALIDADE}}": dados.dataModalidade || "",
        "{{JUSTIFICATIVA}}": dados.justificativaProcedimento || "",
        "{{DATA.DOTACAO}}": dados.dataDotacao || "",
        "{{DATA PED. PARECER}}": dados.dataPedParecer || "",
