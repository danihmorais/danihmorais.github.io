...
# Faixas físicas do PDF que não pertencem ao corpo do documento.
# As coordenadas do pypdf usam origem no canto inferior esquerdo.
HEADER_MARGIN_PT = 84
FOOTER_MARGIN_PT = 72
...
def _page_body_lines(page):
    """Extrai somente o corpo visual da página, descartando cabeçalho e rodapé por posição."""
...
def body_text(reader):
    """Retorna o texto do PDF sem usar texto localizado no cabeçalho/rodapé."""
    pages = [_page_body_lines(page) for page in reader.pages]
    text_pages = ["\n".join(lines) for lines in pages]
    text = "\n\n".join(text_pages)
    return text, text_pages
...
def modality_number(text, detected):
    if detected:
        ...
        m = re.search(
            pat + r".{0,180}?((?:n[ºo°]?|n[uú]mero)?\s*\d{1,4}\s*/\s*\d{4})",
            text,
            re.I | re.S,
        )
        if m:
...
def obj(text):
    """OBJETO = toda a sequência de linhas depois do rótulo até a primeira linha vazia."""
...
def value(text, last=None):
    """Localiza o valor na mesma linha ou nas linhas imediatamente seguintes ao rótulo total."""
    del last
    lines = text.splitlines()
    label_re = re.compile(
        r"total\s+do\s+proponente|valor\s+total|valor\s*\(\s*r\$\s*\)",
        re.I,
    )
    for idx in range(len(lines) - 1, -1, -1):
        ...
        for match in reversed(matches):
            candidate = _money_after_label(line[match.end() :])
            if candidate:
                return candidate
...
def extract(data, filename):
    ...
    if not spaces(text):
        raise ValueError(
            "Não foi possível extrair texto do PDF. O arquivo pode ser escaneado como imagem."
        )
...
async def generate_api(metadata_json: str = Form(...)):
    try:
        ...
        data = await run_in_threadpool(generate, meta)
    except Exception as e:
