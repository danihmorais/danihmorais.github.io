...
def cnpj(v):
    d=re.sub(r"\D", "", v or ""); return f"{d[:2]}.{d[2:5]}.{d[5:8]}/{d[8:12]}-{d[12:]}" if len(d)==14 else (v or "").strip()
def _valid_cnpj(v):
    d=re.sub(r"\D", "", v or "")
...
def _cnpj_candidates(text): return [m.group() for m in CNPJ_RE.finditer(text or "") if _valid_cnpj(m.group())]
...
def _page_body_lines(page):
    ...
    gaps=[rows[i][0]-rows[i+1][0] for i in range(len(rows)-1)]; pg=[g for g in gaps if g>LINE_Y_TOLERANCE_PT]; normal_gap=sorted(pg)[len(pg)//2] if pg else 12.0; blank_gap=max(14.0,normal_gap*1.8); lines=[]
...
def body_text(reader):
    pages=[_page_body_lines(page) for page in reader.pages]; return "\n\n".join("\n".join(x) for x in pages),["\n".join(x) for x in pages]
def raw_text(reader):
    pages=[]
...
def signature_dates(reader,pages):
    ...
    for i,line in enumerate(lines):
        ...
        for j in range(i+1,min(i+5,len(lines))):
            if lines[j].strip():block+=" "+lines[j]
            for m in DATE_RE.finditer(lines[j]):
                after=lines[j][m.end():m.end()+25]; tm=re.search(r"(\d{1,2}:\d{2}(?::\d{2})?)",after)
...
def modality(text):
    ...
    for label,pat in MODALITIES:
        ...
        if m:
            n=NUMBER_RE.search((text or "")[m.end():m.end()+220]); found.append((m.start(),label,number(n.group()) if n else None))
    if not found:return None,None
    _,label,num=min(found,key=lambda x:x[0]); return label,num
def modality_number(text,detected):
    if detected:
        pat=dict(MODALITIES)[detected]; m=re.search(pat+r".{0,220}?((?:n[ºo°]?|n[uú]mero)?\s*\d{1,4}\s*/\s*\d{4})",text or "",re.I|re.S)
        if m:return number(m.group(1))
    ...
    return None
def instrument(text):
    m=INSTRUMENT_RE.search(text or ""); return m.group(1).capitalize() if m else None
def obj(text):
    lines=(text or "").splitlines()
    for i,line in enumerate(lines):
        m=re.match(r"^\s*OBJETO\s*[:\-]?\s*(.*)$",line,re.I)
        if not m:continue
        parts=[]; first=spaces(m.group(1))
...
def contractor(text):
    ...
    if m:return spaces(m.group(1))
    m=re.search(r"^\s*CONTRATADO\s*[:\-]?\s*([^\r\n]+)",text or "",re.I|re.M); return spaces(m.group(1)) if m else None
def cnpj_after_contractor(text):
    m=re.search(r"^\s*(?:CONTRATADA|CONTRATADO)\s*[:\-]?\s*([^\r\n]+)",text or "",re.I|re.M)
    if not m:return None
    candidate=CNPJ_RE.search((text or "")[m.start(1):m.start(1)+500]); return candidate.group() if candidate and _valid_cnpj(candidate.group()) else None
def _money_after_label(text):
    m=CURRENCY_RE.search(text or "")
    if m:return f"R$ {m.group(1)}"
    m=PLAIN_VALUE_RE.search(text or ""); return f"R$ {m.group(0)}" if m else None
def value(text):
    text=text or ""; lines=text.splitlines(); label_re=re.compile(r"(?:total\s+do\s+proponente|valor\s+total|valor\s*\(\s*r\s*\$\s*\))",re.I)
...
def months(text):
    ...
    for m in reversed(list(re.finditer(r"vig[eê]ncia",text,re.I))):
        window=text[m.end():m.end()+420]; v=MONTHS_RE.search(window)
...
def extract(data,filename):
    ...
    if not sd and raw_pages!=pages:sd,sdt=signature_dates(reader,raw_pages)
    mod,mnum=modality(text)
    if not mod and raw:mod,mnum=modality(raw)
    inst=instrument(text) or instrument(raw); contractor_name=contractor(text) or contractor(raw)
    body_cnpjs=_cnpj_candidates(text); raw_cnpjs=_cnpj_candidates(raw); cnpj_candidates=body_cnpjs if len(body_cnpjs)>=2 else raw_cnpjs; selected_cnpj=None
    if len(cnpj_candidates)>=2:selected_cnpj=cnpj_candidates[1]
    elif len(cnpj_candidates)==1:selected_cnpj=cnpj_candidates[0]
    if not selected_cnpj:selected_cnpj=cnpj_after_contractor(text) or cnpj_after_contractor(raw)
    result={"filename":filename,"process_number":process(text) or process(raw),"modality_number":modality_number(text,mod) or modality_number(raw,mod) or mnum,"detected_modality":mod,"modality":mod,"detected_instrument":inst,"instrument":inst,"object":obj(text) or obj(raw),"contractor":contractor_name,"cnpj":cnpj(selected_cnpj) if selected_cnpj else None,"value":value(text) or value(raw),"signature_date":sd,"signature_datetime":sdt,"vigencia_meses":months(text) or months(raw),"error":None}
    missing=[label for label,key in [("assinatura digital","signature_date"),("nº do processo","process_number"),("nº da modalidade","modality_number"),("modalidade","modality"),("instrumento","instrument"),("objeto","object"),("contratada","contractor"),("CNPJ","cnpj"),("valor","value"),("vigência em meses","vigencia_meses")] if not result[key]]
...
def add_months(base,n):
    total=base.year*12+base.month-1+n; y=total//12; mo=total%12+1; nxt=date(y+1,1,1) if mo==12 else date(y,mo+1,1); last=(nxt-date.resolution).day; return date(y,mo,min(base.day,last))
...
def _formatted_replacement(run, value):
    ...
    if rpr is not None:
        caps=rpr.find(qn("w:caps")); small=rpr.find(qn("w:smallCaps"))
        if (caps is not None and caps.get(qn("w:val"), "1") not in ("0","false","off")) or (small is not None and small.get(qn("w:val"), "1") not in ("0","false","off")):
            return value.upper()
    return value
...
def replace_element(el,repls):
    ...
    for p in el.iter(qn("w:p")):
        ...
        for token,repl in repls.items():
            while True:
                full="".join(x.text or "" for x in nodes); first=full.find(token)
                if first<0:break
                end=first+len(token); pos=0; a=b=None; ao=bo=0
                ...
                if a==b:
                    t=nodes[a].text or ""; nodes[a].text=t[:ao]+replacement+t[bo:]
                else:
                    t=nodes[a].text or ""; nodes[a].text=t[:ao]+replacement
...
def generate(meta):
    ...
    d=Document(str(path)); body=d.element.body; sect=body.sectPr; templates=[deepcopy(x) for x in body if x.tag!=qn("w:sectPr")]; instruments=[]
    for i,item in enumerate(docs):
        ...
        modality_value=item.get("modality") or item.get("detected_modality"); instrument_value=item.get("instrument") or item.get("detected_instrument")
        if not modality_value:raise ValueError(f"Modalidade não localizada para {item.get('filename','fornecedor')}.")
        if not instrument_value:raise ValueError(f"Instrumento não localizado para {item.get('filename','fornecedor')}.")
        instruments.append(instrument_value); n=int(item.get("vigencia_meses") or 0)
        if n<=0:raise ValueError(f"Vigência inválida para {item.get('filename','fornecedor')}.")
        final=add_months(initial,n)
        repl={"{{DATA.ASS}}":item.get("signature_date",""),"{{MODALIDADE}}":modality_value,"{{INSTRUMENTO}}":instrument_value,"{{N.PROC}}":item["process_number"],"{{N.PROCESSO}}":item["process_number"],"{{N.MODALIDADE}}":item["modality_number"],"{{OBJETO}}":item.get("object",""),"{{SETOR}}":meta["sector"],"{{CONTRATADA}}":item.get("contractor",""),"{{CNPJ}}":item.get("cnpj",""),"{{VALOR}}":item.get("value",""),"{{VIG.INICIAL}}":initial.strftime("%d/%m/%Y"),"{{VIG.FINAL}}":final.strftime("%d/%m/%Y"),"{{DATA.EXTRATO}}":meta["data_extrato"]}
        if i==0:replace_element(body,repl)
        else:
            insert_at=body.index(sect); body.insert(insert_at,break_next(sect)); insert_at=body.index(sect)
            for t in templates:
                x=deepcopy(t); replace_element(x,repl); body.insert(insert_at,x); insert_at+=1
    out=io.BytesIO(); d.save(out); out.seek(0); return out.getvalue(),instruments
...
async def analyze(files:list[UploadFile]=File(...)):
    async def one(f):
        ...
        except Exception as e:return {"filename":f.filename or "documento.pdf","process_number":None,"modality_number":None,"detected_modality":None,"modality":None,"detected_instrument":None,"instrument":None,"object":None,"contractor":None,"cnpj":None,"value":None,"signature_date":None,"signature_datetime":None,"vigencia_meses":None,"error":str(e)}
    return await asyncio.gather(*(one(f) for f in files))
@app.post("/api/generate")
async def generate_api(metadata_json:str=Form(...)):
    try:meta=json.loads(metadata_json); data,instruments=await run_in_threadpool(generate,meta)
    except Exception as e:raise HTTPException(status_code=400,detail=str(e)) from e
    unique=set(instruments); name="Extratos-Ata.docx" if unique=={"Ata"} else "Extratos-Contrato.docx" if unique=={"Contrato"} else "Extratos-Ata-e-Contrato.docx"
