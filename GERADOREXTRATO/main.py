...
def cnpj(v):
    d = re.sub(r"\D", "", v or "")
    return f"{d[:2]}.{d[2:5]}.{d[5:8]}/{d[8:12]}-{d[12:]}" if len(d) == 14 else v.strip()
...
def _page_body_lines(page):
    ...
    blank_gap = max(14.0, normal_gap * 1.8); lines = []
...
def body_text(reader):
    pages = [_page_body_lines(page) for page in reader.pages]; text_pages = ["\n".join(lines) for lines in pages]
    ...
    for i, line in enumerate(lines):
        ...
        for j in range(i + 1, min(i + 4, len(lines))):
            ...
            for m in DATE_RE.finditer(lines[j]):
                ...
                try: found.append((dt_from(m.group(), tm.group(1) if tm else None), m.group().replace("-", "/").replace(".", "/")))
                except ValueError: pass
...
def modality(text):
    ...
    for label, pat in MODALITIES:
        ...
        if m:
            n = NUMBER_RE.search(text[m.end():m.end()+180]); found.append((m.start(), label, number(n.group()) if n else None))
    if not found: return None, None
    _, label, num = min(found, key=lambda item: item[0]); return label, num
...
def instrument(text):
    m = INSTRUMENT_RE.search(text); return m.group(1).capitalize() if m else None
...
def contractor(text):
    m = re.search(r"^\s*CONTRATADA\s*[:\-]?\s*([^\r\n]+)", text, re.I | re.M); return spaces(m.group(1)) if m else None

def cnpj_after_contractor(text):
    m = re.search(r"^\s*CONTRATADA\s*[:\-]?\s*([^\r\n]+)", text, re.I | re.M)
    if not m: return None
    candidate = CNPJ_RE.search(text[m.start(1):]); return candidate.group(1) if candidate else None
...
def value(text, last=None):
    del last; lines = text.splitlines(); label_re = re.compile(r"total\s+do\s+proponente|valor\s+total|valor\s*\(\s*r\$\s*\)", re.I)
    for idx in range(len(lines)-1, -1, -1):
        ...
        for match in reversed(matches):
            ...
            for next_idx in range(idx+1, min(idx+4, len(lines))):
                if not lines[next_idx].strip(): break
...
def extract(data, filename):
    reader = PdfReader(io.BytesIO(data)); text, pages = body_text(reader)
    if not spaces(text): raise ValueError("Não foi possível extrair texto do PDF. O arquivo pode ser escaneado como imagem.")
    sd, sdt = signature_dates(reader, pages); mod, mnum = modality(text); inst = instrument(text); contractor_name = contractor(text)
    cs = CNPJ_RE.findall(text); selected_cnpj = cs[1] if len(cs) > 1 else cnpj_after_contractor(text)
    if not selected_cnpj and cs: selected_cnpj = cs[0]
    result = {"filename": filename, "process_number": process(text), "modality_number": modality_number(text, mod) or mnum, "detected_modality": mod, "detected_instrument": inst, "object": obj(text), "contractor": contractor_name, "cnpj": cnpj(selected_cnpj) if selected_cnpj else None, "value": value(text), "signature_date": sd, "signature_datetime": sdt, "vigencia_meses": months(text), "error": None}
...
def add_months(base, n):
    total = base.year*12 + base.month - 1 + n; y = total//12; mo = total%12 + 1; nxt = date(y+1,1,1) if mo == 12 else date(y,mo+1,1); last = (nxt-date.resolution).day
...
def generate(meta):
    ...
    effective_instrument = next((item.get("detected_instrument") for item in docs if item.get("detected_instrument") in {"Ata","Contrato"}), meta.get("instrument"))
    for i,item in enumerate(docs):
        ...
        repl={"{{DATA.ASS}}":item.get("signature_date",""),"{{MODALIDADE}}":meta["modality"],"{{INSTRUMENTO}}":effective_instrument,"{{N.PROCESSO}}":item["process_number"],"{{N.MODALIDADE}}":item["modality_number"],"{{OBJETO}}":item.get("object",""),"{{SETOR}}":meta["sector"],"{{CONTRATADA}}":item.get("contractor",""),"{{CNPJ}}":item.get("cnpj",""),"{{VALOR}}":item.get("value",""),"{{VIG.INICIAL}}":initial.strftime("%d/%m/%Y"),"{{VIG.FINAL}}":final.strftime("%d/%m/%Y"),"{{DATA.EXTRATO}}":meta["data_extrato"]}
...
async def analyze(files:list[UploadFile]=File(...)):
    async def one(f):
        ...
        try: return await run_in_threadpool(extract,data,f.filename or "documento.pdf")
            ...
            return {"filename":f.filename or "documento.pdf","process_number":None,"modality_number":None,"detected_modality":None,"detected_instrument":None,"object":None,"contractor":None,"cnpj":None,"value":None,"signature_date":None,"signature_datetime":None,"vigencia_meses":None,"error":str(e)}
...
async def generate_api(metadata_json:str=Form(...)):
    ...
    effective_instrument=next((item.get("detected_instrument") for item in meta.get("documents",[]) if item.get("detected_instrument") in {"Ata","Contrato"}), meta.get("instrument"))
    name="Extratos-Ata.docx" if effective_instrument=="Ata" else "Extratos-Contrato.docx"
