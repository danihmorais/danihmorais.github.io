...
def setup():
    ...
    for b in books:
        active = conn.execute(
            "SELECT id,codigo FROM exemplares WHERE livro_id=? AND ativo=1 ORDER BY id",
            (b["id"],)
        ).fetchall()
        if len(active) < b["quantidade"]:
            for _ in range(len(active) + 1, b["quantidade"] + 1):
                ...
                conn.execute(
                    "INSERT INTO exemplares(livro_id,codigo,criado_em) VALUES(?,?,?)",
                    (b["id"], code, now())
                )
        elif len(active) > b["quantidade"]:
            excess = len(active) - b["quantidade"]
            candidates = conn.execute(
                """SELECT x.id FROM exemplares x
                   WHERE x.livro_id=? AND x.ativo=1
                     AND NOT EXISTS (
                       SELECT 1 FROM emprestimo_exemplares ee
                       WHERE ee.exemplar_id=x.id AND ee.devolvida_em IS NULL
                     )
    ...
    active = conn.execute(
        """SELECT e.id,e.livro_id,e.quantidade FROM emprestimos e
           WHERE e.devolvida_em IS NULL
             AND NOT EXISTS (
               SELECT 1 FROM emprestimo_exemplares x
               WHERE x.emprestimo_id=e.id
             )"""
    ).fetchall()
    for loan in active:
        available = conn.execute(
            """SELECT x.id FROM exemplares x
               WHERE x.livro_id=? AND x.ativo=1
                 AND NOT EXISTS (
                   SELECT 1 FROM emprestimo_exemplares y
                   WHERE y.exemplar_id=x.id AND y.devolvida_em IS NULL
                 )
               ORDER BY x.id LIMIT ?""",
            (loan["livro_id"], loan["quantidade"])
        ).fetchall()
        for x in available:
            conn.execute(
                "INSERT OR IGNORE INTO emprestimo_exemplares(emprestimo_id,exemplar_id) VALUES(?,?)",
                (loan["id"], x["id"])
            )
    conn.commit()
    conn.close()
...
def next_exemplar_code(conn, livro_codigo):
    prefix = f"{livro_codigo}-"
    rows = conn.execute(
        "SELECT codigo FROM exemplares WHERE codigo LIKE ?",
        (prefix + "%",)
    ).fetchall()
    used = set()
    for row in rows:
...
def sync_book_quantity(conn, livro_id):
    count = conn.execute(
        "SELECT COUNT(*) v FROM exemplares WHERE livro_id=? AND ativo=1",
        (livro_id,)
    ).fetchone()["v"]
    conn.execute(
        "UPDATE livros SET quantidade=?, atualizado_em=? WHERE id=?",
        (count, main.now_iso(), livro_id)
    )
    return count
...
class ExemplarCreateIn(BaseModel):
...
def list_exemplares(livro_id: int, user=Depends(main.current_user)):
    ...
    conn = main.db()
    rows = conn.execute(
        """SELECT x.id,x.livro_id,x.codigo,x.ativo,
          CASE WHEN EXISTS(
            SELECT 1 FROM emprestimo_exemplares ee
            WHERE ee.exemplar_id=x.id AND ee.devolvida_em IS NULL
          ) THEN 'Emprestado' ELSE 'Disponível' END status,
          e.id emprestimo_id,p.nome pessoa_nome,e.codigo emprestimo_codigo,
          e.retirada_em,e.prevista_devolucao
          FROM exemplares x
          LEFT JOIN emprestimo_exemplares ee
            ON ee.exemplar_id=x.id AND ee.devolvida_em IS NULL
          LEFT JOIN emprestimos e ON e.id=ee.emprestimo_id
...
def add_exemplares(livro_id: int, data: ExemplarCreateIn, user=Depends(main.current_user)):
    ...
    try:
        ...
        book = conn.execute(
            "SELECT id,codigo,titulo FROM livros WHERE id=? AND ativo=1",
            (livro_id,)
        ).fetchone()
        if not book:
        ...
        for _ in range(data.quantidade):
            code = next_exemplar_code(conn, book["codigo"])
            cur = conn.execute(
                "INSERT INTO exemplares(livro_id,codigo,criado_em) VALUES(?,?,?)",
                (livro_id, code, now())
            )
            created.append({"id": cur.lastrowid, "livro_id": livro_id, "codigo": code, "status": "Disponível", "ativo": 1})

        quantity = sync_book_quantity(conn, livro_id)
        main.log(
            conn,
            "CRIAR",
            "exemplar",
            created[0]["id"],
            f"{data.quantidade} exemplar(es) acrescentado(s) ao livro {book['codigo']}",
            {"livro_id": livro_id, "exemplares": [x["codigo"] for x in created], "quantidade": quantity}
        )
        conn.commit()
        return {"quantidade": quantity, "exemplares": created}
    except Exception:
        conn.rollback()
        raise
    finally:
...
def inativar_exemplar(exemplar_id: int, user=Depends(main.current_user)):
    ...
    try:
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute(
            "SELECT x.*,l.codigo livro_codigo FROM exemplares x JOIN livros l ON l.id=x.livro_id WHERE x.id=? AND x.ativo=1",
            (exemplar_id,)
        ).fetchone()
        if not row:
            raise HTTPException(404, "Exemplar não encontrado.")
        loan = conn.execute(
            """SELECT 1 FROM emprestimo_exemplares
               WHERE exemplar_id=? AND devolvida_em IS NULL LIMIT 1""",
            (exemplar_id,)
        ).fetchone()
        if loan:
            raise HTTPException(409, "Não é possível inativar um exemplar emprestado.")
        conn.execute("UPDATE exemplares SET ativo=0 WHERE id=?", (exemplar_id,))
        quantity = sync_book_quantity(conn, row["livro_id"])
        main.log(
            conn,
            "INATIVAR",
            "exemplar",
            exemplar_id,
            f"Exemplar {row['codigo']} inativado",
            {"livro_id": row["livro_id"], "quantidade": quantity}
        )
        conn.commit()
        return {"ok": True, "quantidade": quantity}
    except Exception:
        conn.rollback()
        raise
    finally:
...
def acervo(user=Depends(main.current_user)):
    ...
    conn = main.db()
    rows = conn.execute(
        """SELECT l.id,l.codigo,l.titulo,l.autor,l.foto,l.quantidade,
          COUNT(x.id) exemplares,
          SUM(
            CASE WHEN x.id IS NOT NULL AND NOT EXISTS(
              SELECT 1 FROM emprestimo_exemplares ee
              WHERE ee.exemplar_id=x.id AND ee.devolvida_em IS NULL
            ) THEN 1 ELSE 0 END
          ) disponiveis
          FROM livros l
          LEFT JOIN exemplares x ON x.livro_id=l.id AND x.ativo=1
          WHERE l.ativo=1
          GROUP BY l.id
...
def create_loan(data: LoanExemplaresIn, user=Depends(main.current_user)):
    ...
    try:
        ...
        rows = conn.execute(
            f"""SELECT id,codigo FROM exemplares
                WHERE livro_id=? AND ativo=1 AND id IN ({qs})
                  AND NOT EXISTS(
                    SELECT 1 FROM emprestimo_exemplares ee
                    WHERE ee.exemplar_id=exemplares.id AND ee.devolvida_em IS NULL
                  )""",
            [data.livro_id, *ids]
        ).fetchall()
        if len(rows) != len(ids):
        ...
        cur = conn.execute(
            "INSERT INTO emprestimos(codigo,livro_id,pessoa_id,quantidade,retirada_em,prevista_devolucao,observacoes) VALUES(?,?,?,?,?,?,?)",
            (code, data.livro_id, data.pessoa_id, len(ids), main.now_iso(), data.prevista_devolucao, data.observacoes.strip())
        )
        ...
        conn.commit()
        loan = conn.execute(
            """SELECT e.*,l.codigo livro_codigo,l.titulo,p.codigo pessoa_codigo,p.nome pessoa_nome
               FROM emprestimos e JOIN livros l ON l.id=e.livro_id JOIN pessoas p ON p.id=e.pessoa_id
               WHERE e.id=?""",
            (loan_id,)
        ).fetchone()
        return dict(loan)
...
def return_exemplares(emprestimo_id:int, data:ReturnExemplaresIn, user=Depends(main.current_user)):
    ...
    try:
        ...
        qs = ",".join("?" for _ in ids)
        links = conn.execute(
            f"SELECT exemplar_id FROM emprestimo_exemplares WHERE emprestimo_id=? AND devolvida_em IS NULL AND exemplar_id IN ({qs})",
            [emprestimo_id, *ids]
        ).fetchall()
